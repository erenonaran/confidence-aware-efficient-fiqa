...
from config import student_test_config
from data import FIQA_dataset
from model import Efficient_FIQA_models
from augmentation import make_aug_transforms
from calibration import fit_logistic_mapping, performance_fit, logistic_func, apply_logistic_function
...
def main():
    gfiqa_path_list = list(glob.glob("/content/data/test/GFIQA-20k/image/*.png"))
    config = TestConfig_EdgeNext(gfiqa_path_list=gfiqa_path_list)

    device = setup_device(config.gpu_ids)

    if config.model_name == 'FIQA_EdgeNeXt_XXS':
        model = Efficient_FIQA_models.FIQA_EdgeNeXt_XXS(pretrained_path=None)
    print("Using model: ", config.model_name)

    try:
        ckpt = torch.load(os.path.join(config.pretrained_path), map_location='cpu', weights_only=False)
    ...
    if config.fitted_curve_path is not None and os.path.exists(config.fitted_curve_path):
        ...
        print("Prediction on G-FIQA dataset...")

        df_data = pd.read_csv(config.gfiqa_csv_path)[['img_name','mos']]
        df_data.columns = ['filename', 'score']
        ...
        for _, row in df_data.iterrows():
            ...
            if not any(image_name.endswith(ext) for ext in ['.jpg', '.jpeg', '.png', '.bmp']):
                image_name += '.png'
            img_path = os.path.join("/content/data/test/GFIQA-20k/image", image_name)
    ...
    if os.path.exists(config.gfiqa_prediction_path):
        scores_df = pd.read_csv(config.gfiqa_prediction_path)
        print(f"CSV file loaded from {config.gfiqa_prediction_path}")
        existing_files = set(scores_df["filename"])
    else:
        print(f"CSV file created at {config.gfiqa_prediction_path}")
        existing_files = set()

    remaining_image_path_list = [
        path for path in config.gfiqa_path_list
    ...
    for batch in tqdm(gfiqa_batches):
        batch_filename_list = []
        batch_image_list = []
        for image_path in batch:
            try:
        ...
        batch_scores_raw = predict_quality(model, batch_image_list, transform, device)
        ...
        if batch_scores_aligned is not None:
            batch_df = pd.DataFrame(
                {"filename": batch_filename_list,
                 "score": batch_scores_aligned,
            })

            batch_df.to_csv(
                config.gfiqa_prediction_path,
                mode="a",
                header=not os.path.exists(config.gfiqa_prediction_path),
                index=False
            )
